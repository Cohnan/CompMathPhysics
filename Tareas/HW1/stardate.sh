#!/bin/bash
# Programa que recibe como un parámetro un año (a partir de 1500) (tiene como default 1996), del cual obtiene un evento histórico obtenido del archivo dado e imprime una tabla con a lo sumo 5 filas con orientación e identificación de estrellas cuya luz sea contemporánea a los eventos (respecto al año 2015).
...
# Consigue archivo con datos de eventos históricos de internet y lo guarda en un archivo oculto
curl -s 'https://raw.githubusercontent.com/ComputoCienciasUniandes/MetodosComputacionales/master/homework/2015-V/HW1/worldhistory.tsv' > .worldhistorytsv

echo $lineaSep
figlet $year
# El awk filtra la columna correspondiente al año.
echo "$(awk 'BEGIN {FS="\t"};{if ($1 == $year) print $2}' .worldhistorytsv)"
echo $lineaSep

# Remueve el archivo con los datos históricos
rm .worldhistorytsv
...
# Consigue archivo con datos de las estrellas de internet y lo guarda en un archivo oculto
curl -s 'https://raw.githubusercontent.com/ComputoCienciasUniandes/MetodosComputacionales/master/homework/2015-V/HW1/hyg.csv' > .hygcsv

printf "%-${cell}.2f%-${cell}.2f%-${cell}d\n" $(awk 'BEGIN {FS=","; i=0};{if (i < 5 && (2015-$year) <= $10 && $10 <= (2015-$year + 1)) {print $8,$9,$2; i++}}' .hygcsv)

# Remueve el archivo con los datos de las estrellas
rm .hygcsv
...
# Mejorar la descarga de los archivos de internet para evitar impresión incompleta de texto
